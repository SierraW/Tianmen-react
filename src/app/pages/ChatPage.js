...
class ChatPage extends React.Component {
    ...
    sendMessage(type = null) {
        if (/$\s*^/.test(this.state.inputMsg)) {
            alert("no empty message allowed");
            this.setState({ inputMsg: "" });
            return;
        }
        em_mashaji(this.props.roomId).add({
            display: true,
    ...
    componentDidMount() {
        em_mashaji(this.props.roomId).orderBy("time").onSnapshot((querySnapshot) => {
            var items = []
            querySnapshot.forEach((doc) => {
                items.push({
                    ...
                    align: doc.data().sender === this.props.uid ? "align-items-end" : "align-items-start",
                    time: datePrettyPrint(doc.data().time.toDate()),
                    type: doc.data().type
                })
                if (!this.state.users[doc.data().sender] && !this.userInfoUpdateArr.includes(doc.data().sender)) {
                    this.userInfoUpdateArr.push(doc.data().sender);
                }
            })
    ...
    keyListner(e) {
        ...
        if (e.keyCode === 13 && e.type === 'keydown') {
            if (this.map[17]) {
                this.setState({ inputMsg: this.state.inputMsg + "\r\n" });
                return;
            }
            e.preventDefault();
            this.sendMessage(1);
    ...
    componentDidUpdate() {
        this.scrollToBottom();
        if (this.userInfoUpdateArr.length != 0) {
            this.updateUserInfo();
        }
    }

    updateUserInfo() {
        var updatedUserMap = {...this.state.users}
        while(this.userInfoUpdateArr.length > 0) {
            const uid = this.userInfoUpdateArr.pop();
        ...
        this.setState({users: updatedUserMap});
    }

    renderMessage(type, msgBody) {
        switch (type) {
            case 1:
        ...
        return (
            <>
                            ...
                            {this.state.messages.map((message, index) => (
                                <div className={`d-flex flex-column mb-5 ${message.align}`} key={index}>
                                    <div className="d-flex align-items-center">
                                        ...
                                        <div>
                                            <a href="#" className="text-dark-75 text-hover-primary font-weight-bold font-size-h6">{ this.state.users[message.sender] }</a>
                                            <span className="text-muted font-size-sm">{message.time}</span>
                                        </div>
                                    </div>

                                    { this.renderMessage(message.type, message.message)}
                ...
                <div className="card-footer align-items-center">
                    ...
                    <textarea className="form-control border-0 p-0" rows="2" placeholder="Type a message" onKeyDown={this.keyListner.bind(this)} onKeyUp={this.keyListner.bind(this)} value={this.state.inputMsg} onChange={(this.handleChange.bind(this))} id="txtIn"></textarea>
                    <div className="d-flex align-items-center justify-content-between mt-5">
                        ...
                        <div>
                            <button type="button" className="btn btn-primary btn-md text-uppercase font-weight-bold chat-send py-2 px-6" onClick={this.sendMessage.bind(this, 1)}>Send</button>
                        </div>
                    </div>
